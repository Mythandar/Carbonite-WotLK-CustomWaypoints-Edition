...
local function GetCategories()
    ...
    for _, category in ipairs(POI.categoryCatalog) do
        categories[#categories + 1] = {
            ...
            enabled = not disabled[category.key],
        }
    end
    return categories
end
...
local function ResolveCategory(query)
    ...
    if wanted == "" then return nil, "No category was specified." end
...
local function GetMainMapGui()
    local map = GetMainMap()
    if map and map.Gui then return map.Gui end
    return Nx and Nx.Map and Nx.Map.Gui or nil
end
...
local function RemoveDisabledPOIs()
    local removed = {}
    local disabled = EnsureSavedData()
    if not Nx or type(Nx.GPOI) ~= "table" then return removed end

    for index = #Nx.GPOI, 1, -1 do
        local value = Nx.GPOI[index]
        if disabled[Normalize(GetPOIName(value))] then
...
function POI:GetCategory(name)
    ...
    if not category then return nil, err end
    return {
        index = category.index,
        name = category.name,
        key = category.key,
...
function POI:IsEnabled(name)
    ...
    return category and IsCategoryEnabled(category.name) or false
end
...
function POI:SetEnabled(name, enabled)
    ...
    local disabled = EnsureSavedData()
    disabled[category.key] = enabled and nil or true
...
function POI:GetSources()
    return {{ id = "builtin", name = "Carbonite built-in POIs", categories = self:GetCategories() }}
end
...
function POI:CreateGUI()
    ...
    for index, category in ipairs(categories) do
        ...
        local x = column == 1 and 28 or 205
        local y = -68 - ((row - 1) * 30)
        self.checks[category.key] = CreateCheck(frame, category, x, y)
    end
...
function POI:RefreshGUI()
    ...
    for _, category in ipairs(self.categoryCatalog) do
        ...
        if check then check:SetChecked(IsCategoryEnabled(category.name)) end
    end
end

function POI:PositionGUI(anchor)
    ...
    if left > screenWidth / 2 then
        frame:SetPoint("BOTTOMRIGHT", anchor, "TOPRIGHT", 4, 4)
    else
        frame:SetPoint("BOTTOMLEFT", anchor, "TOPLEFT", -4, 4)
    end
end

function POI:OpenGUI(anchor)
    local frame = self:CreateGUI()
    if anchor then self:PositionGUI(anchor) end
    self:RefreshGUI()
    frame:Show()
end

function POI:ToggleGUI(anchor)
    local frame = self:CreateGUI()
...
local function ToolbarClick(map, button)
    POI:ToggleGUI(button and button.Frm or nil)
end

function POI:InstallToolbarButton()
    if self.toolbarInstalled then return true end
    local map = GetMainMap()
    if not map or not map.ToB or type(map.ToB.AdB) ~= "function" then return false end

    -- Use Carbonite's own toolbar manager so sizing, spacing, anchoring,
    -- fading and toolbar configuration all match the native map buttons.
    map.ToB:AdB("MapGuide", "POI Filters", nil, ToolbarClick, false)
    map.ToB:Upd()

    local entry = map.ToB.Too and map.ToB.Too[#map.ToB.Too]
    self.toolbarButton = entry and entry.But2 or nil
...
local function PrintList()
    ...
    for _, category in ipairs(GetCategories()) do
        Print(string.format("%-18s %s", category.name, category.enabled and "ON" or "OFF"))
    end
end
...
local function HandleCommand(message)
    ...
    if command == "gui" or command == "window" or command == "options" then
        POI:ToggleGUI(POI.toolbarButton and POI.toolbarButton.Frm or nil)
    ...
    elseif command == "status" then
        ...
        Print(category and (category.name .. " is " .. (IsCategoryEnabled(category.name) and "ON" or "OFF")) or err)
...
local function Install()
    ...
    if not Nx or not Nx.Map or not Nx.Map.Gui or type(Nx.Map.Gui.UZPOII) ~= "function" then return false end
    ...
    _G.CarbonitePOI = POI
    POI.installed = true
    Print("framework v" .. POI.VERSION .. " loaded")
    return true
end
...
loader:SetScript("OnUpdate", function(self, elapsed)
    ...
    if not POI.installed then Install() end
    if POI.installed and POI:InstallToolbarButton() then
